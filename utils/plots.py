...
def plot_one_box_kpt(x, im, color=None, label=None, line_thickness=3, kpt_label=False, kpts=None, steps=2, orig_shape=None):
    ...
    if kpt_label:
        ...
        points_cords = get_points(im=im, kpts=kpts, steps=steps, orig_shape=orig_shape)
        return points_cords
...
def get_points(im, kpts, steps=3, orig_shape=None):
    #Plot the skeleton and keypointsfor coco datatset
    palette = np.array([[255, 128, 0], [255, 153, 51], [255, 178, 102],
                        [230, 230, 0], [255, 153, 255], [153, 204, 255],
                        [255, 102, 255], [255, 51, 255], [102, 178, 255],
    ...
    for kid in range(5):#num_kpts
        ...
        if not (x_coord % 640 == 0 or y_coord % 640 == 0):
            if steps == 3:
                ...
                if conf < 0.5:
                    continue
                array_points.append([x_coord, y_coord])
                cv2.circle(im, (int(x_coord), int(y_coord)), radius, (int(r), int(g), int(b)), -1)
    return array_points